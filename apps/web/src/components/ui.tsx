...
// `className` soma ao estilo base em vez de substituir: antes, espalhar props
// depois do className fazia a classe de quem chama apagar a borda e o foco do
// campo inteiro.
export function Select({ className = '', ...props }: SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      className={`w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 ${className}`}
      {...props}
    />
  );
}
